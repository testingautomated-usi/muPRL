...
class LunarLanderTrainingLogs(TrainingLogs):
    def __init__(
        self,
        is_success: int,
        agent_state: Dict,
        ...
        lander_trajectory: List[float] = None,
        ...
        self.state = state
        self.lander_trajectory = lander_trajectory
    ...
    def to_dict(self) -> Dict:
        config = dict()
        config["height"] = self.config.impl["height"].tolist()
        config["apply_force_to_center"] = self.config.impl["apply_force_to_center"]
        return {
    ...
    def get_testing_image(self) -> np.ndarray:
        assert (
            len(self.lander_trajectory) > 1
        ), "Number of points in trajectory must be > 1. Found: {}".format(
            len(self.lander_trajectory)
        )
        fig = Figure()
        canvas = FigureCanvasAgg(fig)
        ax = fig.add_subplot()
        # ax.axis('off')
        ax.plot(np.ones(shape=len(self.lander_trajectory)), c="blue")
        ax.plot(self.lander_trajectory, c="black")
        ax.plot(np.ones(shape=len(self.lander_trajectory)) * 2, c="blue")
    ...
    def get_dynamic_episode_info(self) -> np.ndarray:
        return np.asarray(self.lander_trajectory)
